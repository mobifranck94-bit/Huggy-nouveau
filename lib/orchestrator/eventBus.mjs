...
export class WindsurfEventBus {
   ...
   * @param {string} [description]
   */
  emitPhaseStart(phase, previousPhase, description) {
    this.emit({
      type: 'phase_start',
      timestamp: Date.now(),
      data: {
   ...
   * @param {string} [description]
   */
  emitPhaseProgress(phase, progress, description) {
    this.emit({
      type: 'phase_progress',
      timestamp: Date.now(),
      data: {
   ...
   */
  emitPhaseComplete(phase, nextPhase) {
    this.emit({
      type: 'phase_complete',
      timestamp: Date.now(),
      data: {
   ...
   * @param {string} [detail]
   */
  emitToolProgress(tool, label, progress, detail) {
    this.emit({
      type: 'tool_progress',
      timestamp: Date.now(),
      data: {
   ...
   * @param {number} [lines]
   */
  emitToolComplete(tool, label, detail, lines) {
    this.emit({
      type: 'tool_complete',
      timestamp: Date.now(),
      data: {
  ...
  /**
   * Emit agent skip event
   * @param {string} agent
   * @param {string} reason
   */
  emitAgentSkip(agent, reason) {
    this.emit({
      type: 'agent_skip',
      agent,
      timestamp: Date.now(),
