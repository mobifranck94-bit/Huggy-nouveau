...
// Plugin that silently drops CSS imports (CSS is injected separately via <style> tag in HTML head)
const cssNoopPlugin = {
  name: 'css-noop',
  setup(build) {
...
// Extract user CSS (src/index.css, src/styles.css, etc.) and inline it as a <style> tag.
// This allows @import for Google Fonts and CSS custom properties to actually take effect.
function extractUserCSS(files) {
  ...
  // Concatenate in priority order: index.css first, then styles.css, then the rest
  const order = (p) => {
    if (/index\.css$/i.test(p)) return 0;
    if (/styles?\.css$/i.test(p)) return 1;
    return 2;
  };
  cssFiles.sort((a, b) => order(a.path) - order(b.path));
...
`;
    ...
    const userCSS = extractUserCSS(files);
    let html = wrapInHTML(js, isEditMode, userCSS);
...
function wrapInHTML(js, isEditMode = false, userCSS = '') {
  ...
  // Inject user CSS (Google Fonts @import + CSS variables) BEFORE the reset so it can take precedence.
  // Without this block, the user's typography & color tokens would never load.
  const userCSSBlock = userCSS.trim() ? `<style>\n${userCSS}\n</style>` : '';
