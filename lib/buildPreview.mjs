...
export async function buildPreviewHTML(files) {
  const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), 'huggy-prev-'));
  try {
    // Write all source files to the temp dir preserving directory structure
...
<head>
  ...
  <style>*,body{margin:0;box-sizing:border-box;font-family:Inter,system-ui,sans-serif}</style>
</head>
<body>
  <div id="root"></div>
  <script type="module">
