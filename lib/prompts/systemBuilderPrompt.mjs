...
- No server-side code in src/, no Node.js APIs, no file system access

FULL-STACK FEATURES (only when explicitly asked):
- AUTH: When the user asks for login/signup, use @supabase/supabase-js with auth.signUp / auth.signInWithPassword / auth.onAuthStateChange. Read VITE_SUPABASE_URL and VITE_SUPABASE_ANON_KEY from import.meta.env. Always include sign-out + protected routing pattern.
- PAYMENTS: When the user asks for payments/subscription/billing, generate:
...
- DATABASE: When the user asks for data persistence, output a \`supabase/migration.sql\` file with CREATE TABLE + RLS policies in addition to the frontend.
- README: For any full-stack app, also emit \`README.md\` with setup steps (env vars, supabase functions deploy, etc.)
