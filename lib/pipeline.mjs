...
export async function runFullPipeline(userPrompt, options = {}) {
  const { onProgress, existingFiles = [], mode = 'build', model = 'claude-sonnet-4-6', projectId = null } = options;
  ...
  // Execute schema in an isolated Postgres schema when a projectId is available
  if (dbaPlan.needsDatabase && projectId) {
    const schemaResult = await executeSchema(dbaPlan, projectId).catch(e => {
      console.warn('[Pipeline] Schema execution failed:', e.message);
      return { skipped: true };
    });
    // Patch supabaseClientCode to target the project-specific schema
